...
$old_error_handler = set_error_handler("app_link_social_scriptcase_error_handler");

function app_link_social_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['app_link_social']['contr_erro']) && $_SESSION['scriptcase']['app_link_social']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_app_link_social->contr_app_link_social->Apl_com_erro = true;
            if ($inicial_app_link_social->contr_app_link_social->NM_ajax_flag)
            {
                $inicial_app_link_social->contr_app_link_social->NM_ajax_info['errList']['geral_app_link_social'][] = $msg;
                return;
            }
            app_link_social_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function app_link_social_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['app_link_social'])  && '' != $_SESSION['scriptcase']['error_icon']['app_link_social'])  ? $_SESSION['scriptcase']['error_icon']['app_link_social']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['app_link_social']) && '' != $_SESSION['scriptcase']['error_close']['app_link_social']) ? $_SESSION['scriptcase']['error_close']['app_link_social'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['app_link_social']) && '' != $_SESSION['scriptcase']['error_span_title']['app_link_social']) ? $_SESSION['scriptcase']['error_span_title']['app_link_social'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['app_link_social']) && '' != $_SESSION['scriptcase']['error_icon_title']['app_link_social']) ? $_SESSION['scriptcase']['error_icon_title']['app_link_social'] : '';
?>
