...
function scEventControl_init(iSeqRow) {
  scEventControl_data["respuesta_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["respuesta_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["respuesta_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_respuesta_' + iSeqRow).bind('blur', function() { sc_form_respuestas_respuesta__onblur(this, iSeqRow) })
                                        .bind('change', function() { sc_form_respuestas_respuesta__onchange(this, iSeqRow) })
                                        .bind('focus', function() { sc_form_respuestas_respuesta__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_respuestas_respuesta__onblur(oThis, iSeqRow) {
  do_ajax_form_respuestas_validate_respuesta_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_respuestas_respuesta__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_respuestas_respuesta__onfocus(oThis, iSeqRow) {
...
function displayChange_block_0(status) {
	displayChange_field("respuesta_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_respuesta_(row, status);
}
...
function displayChange_field_respuesta_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
	$("#id_form_respuestas_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_respuestas')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
