...
function scEventControl_init(iSeqRow) {
  scEventControl_data["agente_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["email_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["agente_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["agente_" + iSeqRow]["change"]) {
    return true;
  }
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_agente_' + iSeqRow).bind('blur', function() { sc_form_agentes_agente__onblur(this, iSeqRow) })
                                     .bind('change', function() { sc_form_agentes_agente__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_agentes_agente__onfocus(this, iSeqRow) });
  $('#id_sc_field_email_' + iSeqRow).bind('blur', function() { sc_form_agentes_email__onblur(this, iSeqRow) })
                                    .bind('change', function() { sc_form_agentes_email__onchange(this, iSeqRow) })
                                    .bind('focus', function() { sc_form_agentes_email__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_agentes_agente__onblur(oThis, iSeqRow) {
  do_ajax_form_agentes_validate_agente_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_agentes_agente__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_agentes_agente__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_agentes_email__onblur(oThis, iSeqRow) {
  do_ajax_form_agentes_validate_email_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_agentes_email__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_agentes_email__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("agente_", "", status);
	displayChange_field("email_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_agente_(row, status);
...
function displayChange_field_agente_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
	$("#id_form_agentes_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_agentes')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
