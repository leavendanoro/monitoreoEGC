...
function scEventControl_init(iSeqRow) {
  scEventControl_data["tipo_mon_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["tipo_mon_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["tipo_mon_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_tipo_mon_' + iSeqRow).bind('blur', function() { sc_form_tipo_mon_tipo_mon__onblur(this, iSeqRow) })
                                       .bind('change', function() { sc_form_tipo_mon_tipo_mon__onchange(this, iSeqRow) })
                                       .bind('focus', function() { sc_form_tipo_mon_tipo_mon__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_tipo_mon_tipo_mon__onblur(oThis, iSeqRow) {
  do_ajax_form_tipo_mon_validate_tipo_mon_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_tipo_mon_tipo_mon__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_tipo_mon_tipo_mon__onfocus(oThis, iSeqRow) {
...
function displayChange_block_0(status) {
	displayChange_field("tipo_mon_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_tipo_mon_(row, status);
}
...
function displayChange_field_tipo_mon_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
	$("#id_form_tipo_mon_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_tipo_mon')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
